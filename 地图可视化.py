...
for i in range(data.shape[0]):
      ...
      maps = (
            Map()
            .add("感染人数",[list(z) for z in zip(location, list(data[i, 1:]))], "长春")
            .set_global_opts(title_opts=opts.TitleOpts(title="长春地图"), visualmap_opts=opts.VisualMapOpts(type_='color',min_=map_min,max_=map_max))
      )
      tl.add(maps, data[i, 0])
tl.add_schema(is_auto_play=True, play_interval=500)
tl.render(path='./2.html')
